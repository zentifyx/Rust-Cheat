...
namespace GUI
{
	...
	void RenderMenu(Vector2& Pos, const Vector2& Size)
	{
		...
		const wchar_t* cheat_name = L"Cheat";

		Render.FillRoundedRectangle(Pos, { MenuHeight, 50 }, ConverToRGB(20, 15, 52), 0);
		{
			Render.FillRectangle(Pos, { MenuHeight, 6 }, ConverToRGB(223, 155, 78));
			Render.RenderString({ Pos.x + 10, Pos.y + 15 }, cheat_name, false, D2D1::ColorF::White, false, true);
			Render.Line({ Pos.x + 90, Pos.y + 15 }, { Pos.x + 90, Pos.y + 40 }, ConverToRGB(40, 40, 64), 2);
		}

		if (page != NULL)
		{
			Render.FillRoundedRectangle({ Pos.x, Pos.y + 65 }, { MenuHeight,  500 }, ConverToRGB(20, 15, 52), 0);
			Render.FillRoundedRectangle({ Pos.x, Pos.y + 105 }, { MenuHeight, 460 }, ConverToRGB(20, 15, 52), 0);
		}

		CurWnd.WndPos = { Pos.x, Pos.y }; CurWnd.Size = Size;
		CurWnd.Pos = { Pos.x, Pos.y + 15.f };
		CurWnd.Togele = { Pos.x, Pos.y + 15.f };
	}
