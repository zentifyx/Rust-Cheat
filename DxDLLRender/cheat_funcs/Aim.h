...
void WeaponPatch()
{
	...
	TestS RPC = (TestS)(GetModBase(L"GameAssembly.dll") + oServerRPC);
	if (Vars::Misc::FastHeal && (weapon->GetID() == 1079279582 || weapon->GetID() == -2072273936) && LocalPlayer.BasePlayer->GetHealth() < 95.f)
	{
		DWORD64 Held = safe_read(weapon + oHeldEntity, DWORD64);
		bool deployed = safe_read(Held + 0x188, bool);
		safe_write(Held + 0x278, 1.f, float);
		auto CurTime = LocalPlayer.BasePlayer->GetTickTime();

		if (deployed && CurTime > Use_Last_FastHeal + 0.5)
		{
			if (IsUsing_FastHeal)
			{
				RPC(Held, L"UseSelf");
